...
export class TransactionsService {
    ...
    async findAll(
        ...
        if (website) {
            query.andWhere('transaction.website ILIKE :website', { website: `%${website}%` });
        }
        if (panel) {
    ...
    async getUniqueClients(
        ...
        const createBaseQuery = () => {
            ...
            if (effectiveWebsite) {
                qb.andWhere('LOWER(transaction.website) = LOWER(:website)', { website: effectiveWebsite.trim() });
            }
            ...
            if (userPanels && userPanels.length > 0) {
                ...
                if (panel) {
                    if (userPanels.some(p => p.toLowerCase() === panel.toLowerCase())) {
                        qb.andWhere('LOWER(transaction.panel) = LOWER(:panel)', { panel: panel.trim() });
                    } else {
        ...
        const countQuery = createBaseQuery()
            .select('COUNT(DISTINCT transaction.client)', 'count');
        ...
        const query = createBaseQuery()
            ...
            .addSelect('SUM(CASE WHEN transaction.type = \'DEPOSIT\' THEN transaction.amount ELSE 0 END)', 'totalDeposits')
            .addSelect('SUM(CASE WHEN transaction.type = \'WITHDRAW\' THEN transaction.amount ELSE 0 END)', 'totalWithdrawals')
            .addSelect('MAX(CASE WHEN transaction.type = \'DEPOSIT\' THEN transaction.date END)::timestamp', 'lastDepositDate')
            .groupBy('transaction.client');

        if (lastDepositDate) {
            query.having('MAX(CASE WHEN transaction.type = \'DEPOSIT\' THEN transaction.date END)::date = :lastDepositDate', { lastDepositDate });
        }

        query.orderBy('transaction.client', 'ASC')
            .offset((page - 1) * limit)
            .limit(limit);
        ...
        const items = results.map(r => ({
            ...
            totalWithdrawals: parseFloat(r.totalWithdrawals || r.totalwithdrawals) || 0,
            lastDepositDate: r.lastDepositDate || r.lastdepositdate || r.last_deposit_date || null,
        }));
