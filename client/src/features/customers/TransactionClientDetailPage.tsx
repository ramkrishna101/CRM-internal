...
const TransactionClientDetailPage = () => {
    ...
    useEffect(() => {
        const fetchClientTransactions = async () => {
            ...
            setError(null);
            try {
                const response = await api.get(`/transactions/clients/${encodeURIComponent(client)}`, {
                    params: { client }
                });
                setData(response.data);
            } catch (err: any) {
                setError(err.response?.data?.message || 'Failed to fetch client transactions');
            } finally {
                setLoading(false);
            }
        };
                ...
                <Grid size={{ xs: 12, md: 8 }}>
                    ...
                    <Paper sx={{ p: 3 }}>
                        ...
                        <TabPanel value={tabValue} index={2}>
                            <Box>
                                <InteractionForm customerId={client || ''} />
                                <Box mt={3}>
                                    <InteractionList customerId={client || ''} />
                                </Box>
                            </Box>
                        </TabPanel>
                    </Paper>
